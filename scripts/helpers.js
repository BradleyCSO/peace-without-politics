var $log, politics_debug_mode, addClass, cl, debounce, debounce_timeout, hasClass, loadUserPreferences, log_timeout;
	...
	return function(callback) {
		return chrome.storage.sync.get(DATA_KEY, function(result) {
			...
			if (!userPreferencesJSONString) {
				...
				if (!_this.userPreferences.hasOwnProperty('removePolitics')) {
					_this.userPreferences.removePolitics = false;
				}
			}
			if (callback) {
				return callback();
...
politics_debug_mode = false;
if (politics_debug_mode) {
	log_timeout = void 0;
